...
func _on_body_entered(body: Node) -> void:
	...
	# parried projectile hurts enemies (including skeleton)
	if _owner == "player" and body.is_in_group("Enemy"):
		if body.has_method("take_damage_from_projectile"):
			body.take_damage_from_projectile(1)
		queue_free()
